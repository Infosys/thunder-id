...
// (currently only PAR) or empty when no proof was sent. When both the dpop_jkt request
...
func ValidateAuthorizationRequestParams(
	...
	if dpopJktParam := params[constants.RequestParamDPoPJkt]; dpopJktParam != "" {
		if !isValidJKTThumbprint(dpopJktParam) {
...
func isValidJKTThumbprint(s string) bool {
	if len(s) != 43 {
		return false
	}
	for i := 0; i < len(s); i++ {
